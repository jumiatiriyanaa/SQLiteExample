package id.ac.polbeng.jumiatiriyana.sqliteexample
...
class StudentDBHelper(context: Context) : SQLiteOpenHelper(
        ...
        try {
            cursor = db.rawQuery(
                "SELECT * FROM " + DBContract.UserEntry.TABLE_NAME +
                        " WHERE " + DBContract.UserEntry.COLUMN_NIM + "=?",
                arrayOf(nim)
            )
    ...
    fun searchStudentByName(searchName: String): ArrayList<StudentModel> {
        val students = ArrayList<StudentModel>()
        val db = writableDatabase
        val cursor: Cursor?
        try {
            cursor = db.rawQuery(
                "SELECT * FROM " + DBContract.UserEntry.TABLE_NAME +
                        " WHERE " + DBContract.UserEntry.COLUMN_NAME + " LIKE ?",
                arrayOf("%$searchName%")
            )
        } catch (e: SQLiteException) {
            db.execSQL(SQL_CREATE_ENTRIES)
